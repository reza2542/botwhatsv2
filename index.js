...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('WG-01', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('WG-01', 'yellow'))
	...
	await aruga.sendText(chat.id, `Yahh... kapasitas grub udh mencapai limit\nMax Group is: ${groupLimit}`).then(() => {
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
	    ...
	    await aruga.sendText(chat.id, `Sorry, WG-01 comes out if the group members do not exceed ${memberLimit} people`).then(() => {
	      aruga.leaveGroup(chat.id)
	      aruga.deleteChat(chat.id)
	    })
	    } else {
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `Hi my friend, Im WG-01. Untuk memunculkan fitur ketik ${prefix}menu`)
        })
	    }
	}
    })
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host) {
            await aruga.sendTextWithMentions(event.chat, `Halo, selamat datang @${event.who.replace('@c.us', '')} \n\nKetik #help untuk memunculkan menu✨`)
        }
        // kondisi ketika seseorang dikick/keluar dari group
        if (event.action === 'remove' && event.who !== host) {
            await aruga.sendTextWithMentions(event.chat, `yahh @${event.who.replace('@c.us', '')}, kenapa aku di tinggal😔`)
        }
    })
